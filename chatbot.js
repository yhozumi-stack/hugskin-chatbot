...
/* ============================================================
   ...
   step の type 一覧:
     ...
     msg          … ボットの発話だけ
     fields       … 入力カード（fields配列に1〜3項目。layout:'2col'で横並び）
...
var SCENARIOS = {
  ...
  standard: [
    ...
    { type: 'fields', key: 'name', intro: 'まず、お名前を教えてください', layout: '2col',
      fields: [
        { key: 'name_last',  label: '姓', placeholder: '例：田中', autocomplete: 'family-name',
          validate: 'required' },
        { key: 'name_first', label: '名', placeholder: '例：花子', autocomplete: 'given-name',
          validate: 'required' },
      ] },
...
async function runStep(i) {
  ...
  if (s.type === 'openingImage') {
    if (CFG.opening.image) {
      var r2 = document.createElement('div');
      r2.className = 'row';
      r2.innerHTML = '<div class="av">💬</div><div class="img-bb"><img src="' + esc(CFG.opening.image) + '" alt=""></div>';
      msgsEl.appendChild(r2); scrollBottom();
    }
    return runStep(i + 1);
  }
  if (s.type === 'msg') {
