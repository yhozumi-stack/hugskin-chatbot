...
- **自動送信(autoSubmit)**(v3.4.0〜): チャット完了時にLPの注文ボタンを自動で押して確認画面まで進む(既定ON)。⚠️**LPが「確認画面を表示しない」設定だと即注文確定になるため、その場合はタグで `autoSubmit: false`**。後払い(同意チェックあり)の時は自動送信せず「同意にチェックして注文ボタンを押してください」の案内になる
