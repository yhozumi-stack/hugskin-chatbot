...
- **登録済みカードの確認(registeredCard・既定ON・v3.30.0〜)**: ログイン中の会員がカードを登録済みで、LPフォームが「登録済みカードを使う」状態(hidden `gateway_card_seq` に値/番号欄がマスク・読取専用)の時、チャットは**カード情報を聞かず「ご登録済みのカード(**** 4242)でお手続きしてよろしいでしょうか？」の確認だけ**にする。「はい」=カード欄に一切触らず登録済みカードで注文/「違うカードにする」=gateway_card_seqを空にして番号欄のロックを解除してから通常のカード入力。**未ログインの新規客ではこの状態にならないため既存LPの挙動は不変**。タグで `registeredCard: false` にすると従来どおり常に入力を聞く。計測 `hs_chat_step_card_registered` / `hs_chat_card_registered_declined`
