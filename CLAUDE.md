...
- **⚠️ HugSkin本番LPは全タグ `cardCvv: false` 必須(2026-07-08決定)**: ZEUS側がセキュリティコード不要設定なのに、ecforceはCVV欄をDOMから消さず**非表示にするだけ**(`#input-cc-cvv` が hidden で残る)。`'auto'` は隠れた欄も検出して聞いてしまい、ボットがCVVを転記するとZEUS設定と不整合で**カードエラーになる**(フォーム手入力はCVV空のまま通るのでボット経由だけ落ちる)。`tags/` の正本には全部入れてあるので、**新LPのタグを作る時も必ず `cardCvv: false` を入れること**。ZEUS設定でCVV必須に戻した時だけ外す
