...
- **🚨 3Dセキュア有効ショップでは `autoSubmit: false` 必須(2026-07-08実測で根本原因特定)**: ZEUSの3Dセキュア(EMV 3DS)が有効なショップでは、正規フロー=「注文ボタン(実クリック)→確認画面→ご注文を完了する→3DS認証」。**ボットのautoSubmit(プログラムクリック`btn.click()`)は信頼されないイベントのため確認画面を経由せず直接POSTになり、3DS未認証のオーソリとしてZEUSが全カードを「104: カード情報を確認してください」で拒否する**(手入力＋実クリックなら同一データで確認画面に進める。全項目JS転記＋実クリックでも確認画面に進めることを実測済み=クリックの信頼性だけが分岐条件)。対処は全チャットタグに `autoSubmit: false`(転記後にお客様が注文ボタンを押す運用)。CVV非表示化と3DS有効化はセットの関係(3DSが立つからCVV省略可)なので、**「CVVを消したらボットだけ104が出る」ようになったらこれを疑う**
