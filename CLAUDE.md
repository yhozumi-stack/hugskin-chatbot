...
**🎯 作業開始の作法(全モデル必須)**: chatbot.js / popup.js / タグ / 計測に触る作業を始める前に、
**このマニュアルの該当レシピ(該当が無ければ「該当レシピなし」と明言)を回答冒頭で引用してから**手を動かすこと。
該当レシピが無い作業は「変更判断ガイド」(下記)を通してから着手する。
これは読み飛ばしによる見当違いな提案・凍結ルール違反を防ぐための決まり(2026-07-29 保積さん指定)。
...
- **登録済みカードの会員は支払いステップが3択になる(registeredCard・既定ON・v3.31.0〜)**: ログイン会員でカード登録済みの場合、「クレジット/後払い」を聞く代わりに**「このカードで進む」「別のカードにする」「(後払い等)」の3択**を出す(カード入力ステップは出さない=「別のカード」を選んだ時だけ入力)。回答バブル・確認画面の✎からの修正も通常ステップと同じに効き、修正時は登録済みカードを覚えているので3択が再度出る(`regCardCache`)。**カード登録が無い/未ログインなら従来どおり2択→カード入力**
- **登録済みカードの確認(registeredCard・既定ON・v3.30.0〜)**: ログイン中の会員がカードを登録済みで、LPフォームが「登録済みカードを使う」状態の時、チャットは**カード情報を聞かず「ご登録済みのカード(**** 4242)でお手続きしてよろしいでしょうか？」の確認だけ**にする。「はい」=カード欄に一切触らず登録済みカードで注文/「違うカードにする」=セレクトを「新しく登録する」に切り替えてから通常のカード入力。
  - **実装の根拠(2026-08-07 ログイン状態の実LPで実測)**: 登録済みカードは `select#card-id`(`order[payment_attributes][source_id]`)で表され、選択肢は `75:************1010 （05/31）` / `0:新しく登録する`。登録済みが選ばれている間は手入力欄(`#input-cc-number`等)が非表示になる。**`gateway_card_seq` は空のまま**なのでこれを見てはいけない**未ログインの新規客ではこの状態にならないため既存LPの挙動は不変**。タグで `registeredCard: false` にすると従来どおり常に入力を聞く。計測 `hs_chat_step_card_registered` / `hs_chat_card_registered_declined`
- **ログイン済み会員の短縮フロー(memberStart・既定OFF・v3.32.0〜)**: 既にログイン中の会員がLPに来た場合、ecforceが注文フォームに登録情報を差し込む(2026-08-07実測)。これを**チャットが何か書き込む前**に検出し、氏名・生年月日・住所・メール・パスワードを聞かずに「**現在、会員としてログインされた状態です👤**」の説明+登録情報カード→[この内容で進む]/[お届け先を変更する]→支払い→確認画面 に短縮する
  - 検出条件は「氏名+(郵便番号 or 住所)が入っている」**かつ「ログイン中にしか現れない印(登録済みカードのセレクト or パスワード欄が出ない)がある」**こと。さらに**エラー弾き返し画面(payment_error_code有り)と、このブラウザで既に送信済みの場合は発動しない**
  - **⚠️v3.32.0の誤検知(2026-08-07 実LPで確認・v3.32.1で修正済)**: ecforceは**弾き返し画面や再訪問時に、お客様が入力した値をフォームに再表示する**。氏名・住所が入っているだけで判定すると、未ログインの人に「ログインされた状態です」と出してしまう。**値が入っている=ログイン中ではない**
  - **お客様が変更しない限りフォームの登録値は上書きしない**(答えは表示・確認画面用に保持するだけ。同値なので転記されても無害)
  - ログイン中はメール既登録チェック(emailDupCheck)を自動で抑止(自分のメールに「既に登録があります」は的外れなため)
  - LPがログイン中もパスワード欄を必須表示するテーマ向けに、**パスワードだけ1問聞く保険**あり(欄が非表示/入力済みなら聞かない)
  - 計測 `hs_chat_member_start_detected / _view / _ok / _change_addr`。動作確認は `preview/member_test.html`(`?logout`=未ログイン / `?nocard`=カード未登録 / `?pw`=パスワード必須テーマ)
- **会員のチャット内ログイン(memberLogin・既定OFF・v3.29.0〜)**: タグに `memberLogin: true` を書いたLPだけ、既登録検知時の案内が「ログイン画面を開く(別タブ)」ではなく**チャット内でパスワードを聞いてその場でAJAXログイン**になる(Teaflex/form-plusのUX移植。2026-08-07に実LPで動作観察)。成功→ページ再読込→ecforceがログイン済みフォームを描画(氏名住所は登録情報)→チャットは**支払い方法から再開**して注文完了。パスワード誤りはチャット内でエラー・再入力可、「忘れた方はこちら」は`forgotUrl`(既定=hugskin.shopの再設定ページ)。計測 `hs_chat_member_login_try/ok/ng/resume`。emailDupCheckとセット推奨
- **メール入力時点の既登録チェック(emailDupCheck・既定OFF・v3.28.4〜/v3.29.1でAPI方式に刷新)**: タグに `emailDupCheck: true` を書いたLPだけ、チャットでメール確定と同時に**ecforceの既登録チェックAPIを直接問い合わせ**、既登録なら**送信を待たずその場で**会員案内(ログイン/別のメールアドレスで入力する=1問修正でメールだけ再入力)を出す。案内中は後続の質問を描画しない
  - API(2026-08-07に実LPで特定・`emailCheckUrl`で変更可): `GET /lp/email?fieldId=email&fieldValue=<メール>` → `["email",false]`=**既登録** / `["email",true]`=未登録。LPのメール欄の `validate[...ajax[emailCheck]]` が内部で叩いているものと同じ
  - **⚠️v3.29.0以前のDOM文言検知は当てにならない**: フォームにchangeを発火するとAPIは走るが、validationEngineがエラープロンプトを描画しないケースがあり(2026-08-07 実LPで確認)取りこぼした。API方式は**フォームに一切触らない**ので再描画事故もなく、hideForm併用も可。APIが無いLPでは旧DOM方式へ自動フォールバック
- **メール既登録の自動検知**(v3.4.0〜/v3.28.3で遅延検知を追加): 送信後にecforceが「既に登録されています」で弾いて戻ってきた画面を検知すると、チャットが自動で開いて「ログイン画面を開く/別のメールで入力する」を案内する(全シナリオ共通・設定不要)。**エラーバナーを読込後にJSで表示するLPテーマ**(NP本番LPで2026-08-07に実際に取りこぼし発生)向けに、初期化から約30秒間の再判定も行う=遅れて出たバナーも拾い、未入力ならログイン案内に差し替える(入力開始後は邪魔しない)
...
- **⚠️ HugSkin本番LPは全タグ `cardCvv: false` 必須(2026-07-08決定)**: ZEUS側がセキュリティコード不要設定なのに、ecforceはCVV欄をDOMから消さず**非表示にするだけ**(`#input-cc-cvv` が hidden で残る)。`'auto'` は隠れた欄も検出して聞いてしまうので明示的にOFF。`tags/` の正本には全部入れてあるので、**新LPのタグを作る時も必ず `cardCvv: false` を入れること**
- **🚨 3Dセキュア有効化後に「ボット経由だけカード104」が出たら、原因はカード欄のイベント不足(2026-07-09に実コードで根本原因を特定・確定)**:
  - ZEUSの3Dセキュア(EMV 3DS)が有効なショップでは、**カード欄(番号/月/年/名義)を `input` イベントだけで転記すると、autoSubmit時にZEUSの3DS検証状態が立たず全カードが「104: カード情報を確認してください」で弾かれる**(クライアント側で拒否・3DS未到達)。手入力だと通るのは、人の手入力が `focus/keydown/keyup/change/blur` を発火してZEUSの内部検証フラグを立てるから
  - **isTrusted(submit clickの信頼性)は無関係**。実測で「全項目JS転記＋カード欄をリッチイベントで転記＋**プログラムクリック**(`btn.click()`)」でもZEUSの3DS(`/payments/zeus_3ds`→`/lp/new`)まで到達することを確認。分岐条件はカード欄のイベントの豊富さだけ(当初の「クリックの信頼性が原因」説は座標スケール0.817のクリックずれによる誤診だった)
  - **対処(実装済み・v3.24.0/commit c2260c7)**: `chatbot.js` の `typeCardValue`/`setCardField` がカード欄を人の手入力に近いイベント列(text=focus→1文字ずつkeydown/input/keyup→change→blur、select=focus→change→blur)で転記する。**silent再固定ではカード欄を触らない**(イベントなしの上書きが3DS検証状態を壊すため)。これで **autoSubmitは維持のまま**チャット内完結→通常の3DSフローに乗る(`autoSubmit: false`は不要・不採用)
  - **⚠️「CVVを消したらボットだけ104が出る」ようになったらこれを疑う**(CVV非表示化と3DS有効化はセットの関係=3DSが立つからCVV省略可。転記のイベント不足が顕在化したのがこのタイミングだっただけ)。ダミーカード4111での正常到達点は「500: 3Dセキュア認証に対応していないカード」(=3DSまで到達した証拠。実カードなら3DS認証画面へ)
...
#### closeConfirm用画像の発注メモ(デザイナーにそのまま渡せる・LP離脱POPとは別物)
> - **用途**: チャットの×を押した人を引き止める安心訴求ビジュアル
> - **サイズ**: 幅680px(スマホ表示≒340pxの@2x)。縦は内容次第(正方形〜やや横長が収まりやすい)
> - **CTAボタンは描かない・角丸なし**。「閉じる/チャットに戻る」ボタンとタイトル白部分は**システム側が固定で描画**する(画像はその下に入る純粋なビジュアル)
> - 画像タップ=「チャットに戻る」扱い。誘導ボタンを画像に焼き込む必要はない
> - **文言(HugSkinの条件に合わせた安全表現)**:
>   - 「購入回数のお約束なし」
>   - 「次回発送予定日の10日前までに休止・変更・解約可能」
>   - ※「いつでもOK」等の曖昧・過大な表現は避ける
- ⚠️**期限・条件(10日前/休止・変更・解約の別)は、発注前に最新のLP特商法記載・Brand Sheetで最終確認**する(参考LP=Teaflexは7日前でHugSkinとは違う。数字を転用しない)
- 画像ができたら `closeConfirm.image` をHugSkin用画像URLに差し替えるだけ(現在はテスト用GIF `popup_test_teaflex.gif` が入っている)

### レシピ18: 与信NGリカバリー(paymentFallback・LP個別・v3.26.0〜)
後払いの**与信落ちでecforceがLPに弾き返した画面**をチャットが自動検知し、自動で開いて
「クレジットカードでのご注文」に切替→**カード情報だけ再入力→自動送信**で復旧させる。
住所・氏名等はecforceがフォームに再表示した値をそのまま使うので再質問しない。
タグに追加するだけ(**既定OFF**なので、書いたLPだけで有効):
```js
paymentFallback: {},   // 最小はこれだけでON。文言を変えたい時だけ↓
/* paymentFallback: {
...
```
- **前提: ecforce側の後払いが「リアルタイム(同期)与信」設定であること**(注文成立後の非同期与信NGではLPに戻ってこないため検知できない)。HugSkinをNP後払いリアルタイムwiz等に切り替えたタイミングで各LPのタグに入れる
- **検知の仕組み**(2026-07-17にBefasの与信NG実画面で実測・確定):
  - 最優先アンカー = 弾き返し画面のhidden `input[name="payment_error_code"]` に値が入る(実測値`NG999`。表示文言に依存しない)
  - 保険 = **表示中の**alert系要素の「与信審査が通りません」文言(実画面では `.alert_ec.alert-danger_ec` に「20 : 与信審査が通りませんでした。…」。非表示のalertは無視する)
  - 弾き返しURLは `/lp/new?at=lp-form&pm=<失敗した支払いID>&u=…`。**pm=がクレジットのIDの時は発動しない**(クレカ決済失敗に「クレカでお願いします」と案内する矛盾の防止)
- **パスワードの復旧チェーン**(ecforceは弾き返し画面でパスワードだけ再表示しない):
  ①フォームに欄が無いLP→何もしない ②欄に値が残っていれば→そのまま ③**sessionStorage引き継ぎ**(送信直前に難読化して預けた値を自動復元。1回使い切り・15分期限・与信NG以外のページ読込で即破棄。**カード情報は絶対に預けない**) ④タグのskip設定があれば→新値を自動生成 ⑤どれも無ければ→「パスワードのみ再入力」を1問だけ挟む
- 確認画面(summary)は挟まない(案内→カード入力→即送信の最短導線。**例外=下記の代引き選択時**)。hideForm併用LPではNG画面でも隠したまま(×で閉じれば従来どおり再表示)
- **代引きも選ばせる(daibiki・v3.35.0〜・既定OFF)**: `paymentFallback: { daibiki: true }` にすると、与信NG画面で**LPフォームに代引きの選択肢(文言に「代引」か「代金引換」を含む)がある時だけ**「クレジットカード/代金引換」の2択を出す(無ければ従来どおりクレカのみ=自動判定。**ecforce側で代引きを支払い方法に含めておくのが前提**)
  - クレジット→従来どおりカード入力→即送信。**代金引換→手数料で合計が変わるので、ecforce再計算後の合計(手数料行付き)・注意喚起文・同意チェックを載せた確認画面を挟み「この内容で注文する」で送信**(金額が変わる以上、最終確認の表示が要るため。クレカ側は金額が変わらないので従来どおり確認なし)
  - 文言はタグで上書き可: `choiceMsg`(2択の質問文) / `daibikiMsg`(代引き確認画面の口上) / `msg`(案内。daibiki有効時の既定は末尾が「別のお支払い方法でのお手続きをお願いいたします」。**`msg`を「クレジットカードで〜」の自前文言で上書きしているLPは daibiki 追加時に文言も直すこと**)。判定語を変えたい時は `daibiki: '代金引換'` のように文字列で指定
  - 計測: `hs_chat_payment_ng_credit` / `hs_chat_payment_ng_daibiki`(2択でどちらを選んだか)→ `hs_chat_payment_ng_retry`(送信)。**代引き回収率 = retry ÷ daibiki**。この確認画面は通常ファネルの summary_view に混ぜない
  - 動作確認: `preview/payment_ng_test.html?ng=1&daibiki=1`(2択→代引き確認画面→送信 / クレカ→カード→送信) / `&nodb=1`(フォームに代引きなし→クレカのみ) / `?ng=1`(daibiki未設定→2択が出ない=既定OFF)
- 計測: `hs_chat_payment_ng_detected`(検知) / `hs_chat_payment_ng_retry`(カード入力完了→再送信)が自動で飛ぶ(既存パイプラインにそのまま乗る)。**回収率 = auto_submit ÷ detected** で見る
- 文言の注意: 既定文言は「審査」に触れないぼかし表現にしてある(NP等の決済会社の規約対策)。「与信審査が通りませんでした」と明言したい場合はタグの`msg`で上書き
- 動作確認: `preview/payment_ng_test.html`(ページ上部にテストモード一覧。`?ng=1`=PW再質問 / `&skip=1`=skip再生成 / `&nopw=1`=PW欄なし / `&held=1`=引き継ぎ / `&pm=21`=非発動 / `&off=1`=既定OFF / `&daibiki=1`=代引き2択 / `&nodb=1`=フォームに代引きなし / パラメータ無し=チャット完走→弾き返しの一気通貫テスト)
...
1. **冒頭の定期コース条件案内**: `scenario: 'formplus_np'` を指定。formplusと同一フローの冒頭(最初の入力の前)に
   ...
   - 案内の位置は3種(タグの `scenario` で選ぶだけ・push不要): `formplus_np`=ローディング演出→**案内**→画像 / `formplus_np2`=ローディング演出→画像→**案内** / `formplus_np3`=**案内**→ローディング演出→画像(案内を一番最初に出す)
