...
- **メール入力時点の既登録チェック(emailDupCheck・既定OFF・v3.28.4〜)**: タグに `emailDupCheck: true` を書いたLPだけ、チャットでメール確定→転記直後にメール欄のchange/blurを発火してecforceの既登録チェックAJAXを誘発し、「＊ 既に使われています」表示を検知したら**送信を待たずその場で**ログイン案内(ログイン画面を開く/別のメールアドレスで入力する=1問修正でメールだけ再入力)を出す。⚠️hideForm併用不可(エラーが画面に出ず検知できない)。計測は `hs_chat_dup_email_inline`
